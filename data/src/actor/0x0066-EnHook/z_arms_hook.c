...
void ArmsHook_Init(Actor* thisx, PlayState* play) {
    ...
    if (LINK_IS_ADULT) {
        this->requiredObjBankIndex = Object_GetIndex(&play->objectCtx, OBJECT_LINK_BOY);
    } else {
        this->requiredObjBankIndex = Object_GetIndex(&play->objectCtx, OBJECT_LINK_CHILD);
    }
}
...
void ArmsHook_Draw(Actor* thisx, PlayState* play) {
    ...
    if ((player->actor.draw != NULL) && (player->rightHandType == PLAYER_MODELTYPE_RH_HOOKSHOT)) {
        ...
        // Spike Display
        if (LINK_IS_ADULT){
            gSPDisplayList(POLY_OPA_DISP++, gPlayasAdult_DlHookshotSpike);
        }else{
            gSPDisplayList(POLY_OPA_DISP++, gPlayasChild_DlHookshotSpike);
        }
        ...
        //Chain Display
        if (LINK_IS_ADULT){
            gSPDisplayList(POLY_OPA_DISP++, gPlayasAdult_DlHookshotChain);
        }else{
            gSPDisplayList(POLY_OPA_DISP++, gPlayasChild_DlHookshotChain);
        }
