 ...
 *      -- YY: Type - 0x00 Green, 0x01 Red, 0x03 Yellow, 0x0A Blue, 0x04 Fire, 0x06 Ice, 0x08 Electric
 */
...
#define ACT_ID 0x0003
#define OBJ_ID 0x0005
...
typedef enum {
    ...
    /* 0x1 */ ZOL_DMGEFF_SWORD = 0x1,
    /* 0xE */ ZOL_DMGEFF_FIRE = 0xE,
...
static DamageTable sDamageTable[] = {
    ...
    /* Hammer swing  */ DMG_ENTRY(1, ZOL_DMGEFF_SWORD),
    /* Hookshot      */ DMG_ENTRY(0, ZOL_DMGEFF_DEFAULT),
    /* Kokiri sword  */ DMG_ENTRY(1, ZOL_DMGEFF_SWORD),
    /* Master sword  */ DMG_ENTRY(1, ZOL_DMGEFF_SWORD),
    /* Giant's Knife */ DMG_ENTRY(2, ZOL_DMGEFF_SWORD),
    /* Fire arrow    */ DMG_ENTRY(1, ZOL_DMGEFF_FIRE),
    ...
    /* Fire magic    */ DMG_ENTRY(0, ZOL_DMGEFF_FIRE),
    ...
    /* Kokiri jump   */ DMG_ENTRY(2, ZOL_DMGEFF_SWORD),
    /* Giant jump    */ DMG_ENTRY(8, ZOL_DMGEFF_SWORD),
    /* Master jump   */ DMG_ENTRY(4, ZOL_DMGEFF_SWORD),
...
static InitChainEntry sInitChain[] = {
    ICHAIN_S8(naviEnemyId, 0x93, ICHAIN_CONTINUE),
...
void EnZol_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params == ZOL_SPLIT || this->actor.params == ZOL_SPLIT_FIRE || this->actor.params == ZOL_SPLIT_ICE || this->actor.params == ZOL_SPLIT_ELEC) {
        this->scale = 0.04f;
    } else {
        this->scale = 0.08f;
    }
    ...
    switch (ENZOL_TYPE) {
        ...
        break;
        case ZOL_YELLOW:
            thisx->naviEnemyId = 0xD9;
            thisx->colChkInfo.health = 4;
        break;
        case ZOL_FIRE:
            thisx->naviEnemyId = 0xAB;
            this->collider.base.colType = COLTYPE_HIT7; // red blood hit effect
        ...
        break;
        case ZOL_ICE:
            thisx->naviEnemyId = 0xAC;
            this->collider.base.colType = COLTYPE_HIT0; // blue blood hit effect
            thisx->colChkInfo.health = 6;
        ...
        break;
        case ZOL_ELEC:
            thisx->naviEnemyId = 0xAD;
            thisx->colChkInfo.health = 6;
            this->collider.info.toucher.effect = 3;
        break;
        case ZOL_SPLIT_ELEC:
            thisx->naviEnemyId = 0xAD;
            this->collider.dim.radius = 26;
        ...
        break;
        case ZOL_BLUE:
            thisx->naviEnemyId = 0xFE;
            thisx->colChkInfo.health = 1;
            this->collider.base.colType = COLTYPE_HIT0;
        break;
    }

    if ((ENZOL_TYPE != ZOL_SPLIT) || (ENZOL_TYPE != ZOL_SPLIT_FIRE) || (ENZOL_TYPE != ZOL_SPLIT_ICE) || (ENZOL_TYPE != ZOL_SPLIT_ELEC)) {
        switch (ENZOL_SPAWN) {
            case ZOL_NONE:
...
void EnZol_Grow(EnZol* this, PlayState* play) {
    func_80078914(&this->actor.world.pos, NA_SE_EN_MOFER_CORE_ROLL - SFX_FLAG); // play sound continuously, every frame
    if (ENZOL_TYPE == ZOL_FIRE || ENZOL_TYPE == ZOL_ICE || ENZOL_TYPE == ZOL_ELEC) {
        SoundEffect_PlayHeld(SOUND_ENEMY_MAD_JELLY_RISE, 1.0f, 1.0f, 1.0f, &this->actor.world.pos, 1.0f, 1000.0f, &this->actor); // play sound continuously, every fram
    }
...
void EnZol_Recoil(EnZol* this, PlayState* play) {
    ...
    if ((this->actor.speedXZ == 0.0f) && (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
        if (this->timer > 0) {
            this->timer--;
        } else {
            this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnZol_DeathCry(EnZol* this, PlayState* play) {
    ...
    if (this->actor.scale.y >= this->scale + 0.08f) {
        ...
        if (ENZOL_TYPE == ZOL_SPLIT || ENZOL_TYPE == ZOL_RED || ENZOL_TYPE == ZOL_FIRE || ENZOL_TYPE == ZOL_SPLIT_FIRE) { // red one, split, fire
            effectScale = 100;
        ...
        } else if (ENZOL_TYPE == ZOL_YELLOW || ENZOL_TYPE == ZOL_ELEC || ENZOL_TYPE == ZOL_SPLIT_ELEC) { // yellow, electric
            effectScale = 100;
            effectPrimColor.r = 255;
            effectPrimColor.g = 255;
            effectEnvColor.r = 70;
            effectEnvColor.g = 70;
            Item_DropCollectibleRandom(play, &this->actor, &this->actor.world.pos, 0x10);
        } else if (ENZOL_TYPE == ZOL_ICE || ENZOL_TYPE == ZOL_SPLIT_ICE) { // ice
            effectScale = 100;
            effectPrimColor.g = 180;
            effectPrimColor.b = 255;
            effectEnvColor.g = 0;
            effectEnvColor.b = 70;
            Item_DropCollectibleRandom(play, &this->actor, &this->actor.world.pos, 0x10);
        } else if (ENZOL_TYPE == ZOL_BLUE) { // blue
            effectScale = 100;
            effectPrimColor.r = 180;
            effectPrimColor.g = 180;
            effectPrimColor.b = 255;
            effectEnvColor.r = 0;
            effectEnvColor.g = 0;
    ...
    if (this->collider.base.acFlags & AC_HIT && this->actionFunc != EnZol_Recoil && this->state != ZOL_WAIT  && this->state != ZOL_DEATH) {
        ...
        if ((thisx->colorFilterTimer == 0) || ((thisx->colorFilterParams & 0x4000) == 0)) {
            if (this->actor.colChkInfo.damageEffect == ZOL_DMGEFF_ICE && this->state != ZOL_FROZEN) {
                if (this->actor.colChkInfo.damageEffect == ZOL_DMGEFF_ICE && (ENZOL_TYPE == ZOL_ICE || ENZOL_TYPE == ZOL_SPLIT_ICE)) {
                ...
                if (this->actor.colChkInfo.damageEffect == ZOL_DMGEFF_SWORD && (ENZOL_TYPE == ZOL_ELEC || ENZOL_TYPE == ZOL_SPLIT_ELEC)) {
                func_8002F698(play, &this->actor, 0.0f, this->actor.yawTowardsPlayer, 0.0f, 3, 0x10);
                }
                ...
                } else if (ENZOL_TYPE == ZOL_FIRE) {
                    Actor* childZol;

                    this->actor.params = ZOL_SPLIT_FIRE;
                    this->scale = 0.04f;
                    Actor_SetScale(&this->actor, this->scale);
                    this->collider.dim.radius = 26;
                    this->collider.dim.height = 35;
                ...
                } else if (ENZOL_TYPE == ZOL_ICE) {
                    Actor* childZol;

                    this->actor.params = ZOL_SPLIT_ICE;
                    this->scale = 0.04f;
                    Actor_SetScale(&this->actor, this->scale);
                    this->collider.dim.radius = 26;
                    this->collider.dim.height = 35;
                ...
                } else if (ENZOL_TYPE == ZOL_ELEC) {
                    Actor* childZol;

                    this->actor.params = ZOL_SPLIT_ELEC;
                    this->scale = 0.04f;
                    Actor_SetScale(&this->actor, this->scale);
                    this->collider.dim.radius = 26;
                    this->collider.dim.height = 35;
...
void EnZol_Draw(Actor* thisx, PlayState* play) {
    ...
    switch (ENZOL_TYPE) {
        ...
        break;
        case ZOL_YELLOW:
            gDPSetEnvColor(POLY_XLU_DISP++, 70, 70, 0, 255);
            gDPSetPrimColor(POLY_XLU_DISP++, 0x00, 0x80, 255, 255, 180, 255);
        break;
        case ZOL_BLUE:
            gDPSetEnvColor(POLY_XLU_DISP++, 0, 0, 70, 255);
            gDPSetPrimColor(POLY_XLU_DISP++, 0x00, 0x80, 180, 180, 255, 255);
        break;
        case ZOL_FIRE:
        case ZOL_SPLIT_FIRE:
            gDPSetEnvColor(POLY_XLU_DISP++, 150, 120, 0, 255);
            gDPSetPrimColor(POLY_XLU_DISP++, 0x00, 0x80, 255, 200, 80, 255);
        break;
        case ZOL_ICE:
        case ZOL_SPLIT_ICE:
            gDPSetEnvColor(POLY_XLU_DISP++, 0, 120, 150, 255);
            gDPSetPrimColor(POLY_XLU_DISP++, 0x00, 0x80, 80, 200, 255, 255);
        break;
        case ZOL_ELEC:
        case ZOL_SPLIT_ELEC:
            gDPSetEnvColor(POLY_XLU_DISP++, 150, 150, 0, 255);
            gDPSetPrimColor(POLY_XLU_DISP++, 0x00, 0x80, 255, 255, 80, 255);
        break;
    }

    gSPDisplayList(POLY_XLU_DISP++, 0x060014c0);

    CLOSE_DISPS(play->state.gfxCtx, __FILE__, __LINE__);

    Gfx_DrawDListOpa(play, 0x06001890);
}
