...
Asm_SymbolAlias("__z64_init", 0x0000000);
Asm_SymbolAlias("__z64_dest", 0x0000000);
Asm_SymbolAlias("__z64_updt", KaleidoScope_Update);
Asm_SymbolAlias("__z64_draw", KaleidoScope_Draw);
...
Gfx* KaleidoScope_QuadTextureIA4(Gfx* gfx, void* texture, s16 width, s16 height, u16 point) {
    gDPLoadTextureBlock_4b(
        gfx++, texture, G_IM_FMT_IA, width, height, 0, G_TX_NOMIRROR | G_TX_WRAP,
...
Gfx* KaleidoScope_QuadTextureIA8(Gfx* gfx, void* texture, s16 width, s16 height, u16 point) {
    gDPLoadTextureBlock(
        gfx++, texture, G_IM_FMT_IA, G_IM_SIZ_8b, width, height, 0, G_TX_NOMIRROR | G_TX_WRAP,
        G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD
    );
    gSP1Quadrangle(gfx++, point, point + 2, point + 3, point + 1, 0);
...
void KaleidoScope_MoveCursorToSpecialPos(PlayState* play, u16 specialPos) {
    ...
    Audio_PlaySfxGeneral(
        NA_SE_SY_DECIDE, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
...
void KaleidoScope_DrawQuadTextureRGBA32(GraphicsContext* gfxCtx, void* texture, u16 width, u16 height, u16 point) {
    ...
    gDPLoadTextureBlock(
        POLY_OPA_DISP++, texture, G_IM_FMT_RGBA, G_IM_SIZ_32b, width, height, 0,
...
void KaleidoScope_SwitchPage(PauseContext* pauseCtx, u8 pt) {
    ...
    if (!pt) {
        ...
        Audio_PlaySfxGeneral(
            NA_SE_SY_WIN_SCROLL_LEFT, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
            &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb
        );
        ...
        Audio_PlaySfxGeneral(
            NA_SE_SY_WIN_SCROLL_RIGHT, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
            &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb
        );
...
void KaleidoScope_DrawCursor(PlayState* play, u16 pageIndex) {
    ...
    if (
        (((pauseCtx->unk_1E4 == 0) || (temp == 8)) && (pauseCtx->state == 6)) ||
        ...
        if (pauseCtx->pageIndex == pageIndex) {
            ...
            gDPSetCombineLERP(
                POLY_OPA_DISP++, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0,
                PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0
            );
            gDPSetPrimColor(
                POLY_OPA_DISP++, 0, 0, sCursorColors[pauseCtx->cursorColorSet >> 2][0],
            ...
            for (i = j = 0; i < 4; i++, j += 4) {
                gDPLoadTextureBlock_4b(
                    POLY_OPA_DISP++, sCursorTexs[i], G_IM_FMT_IA, 16, 16, 0,
...
Gfx* KaleidoScope_DrawPageSections(Gfx* gfx, Vtx* vertices, void** textures) {
    ...
    while (j < 32) {
        gDPPipeSync(gfx++);
        gDPLoadTextureBlock(
            gfx++, textures[i], G_IM_FMT_IA, G_IM_SIZ_8b, 80, 32, 0, G_TX_NOMIRROR | G_TX_WRAP,
            G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD
        );
...
void KaleidoScope_DrawPages(PlayState* play, GraphicsContext* gfxCtx) {
    ...
    if ((pauseCtx->state < 8) || (pauseCtx->state > 0x11)) {
        ...
        if (pauseCtx->pageIndex) { // pageIndex != PAUSE_ITEM
            ...
            gSPMatrix(
                POLY_OPA_DISP++, Matrix_NewMtx(gfxCtx, "../z_kaleido_scope_PAL.c", 1173),
                G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
            );
            
            POLY_OPA_DISP = KaleidoScope_DrawPageSections(
                POLY_OPA_DISP, pauseCtx->itemPageVtx,
        ...
        if (pauseCtx->pageIndex != PAUSE_EQUIP) {
            ...
            gSPMatrix(
                POLY_OPA_DISP++, Matrix_NewMtx(gfxCtx, "../z_kaleido_scope_PAL.c", 1196),
                G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
            );
            
            POLY_OPA_DISP = KaleidoScope_DrawPageSections(
                POLY_OPA_DISP, pauseCtx->equipPageVtx,
        ...
        if (pauseCtx->pageIndex != PAUSE_QUEST) {
            ...
            gSPMatrix(
                POLY_OPA_DISP++, Matrix_NewMtx(gfxCtx, "../z_kaleido_scope_PAL.c", 1220),
                G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
            );
            
            POLY_OPA_DISP = KaleidoScope_DrawPageSections(
                POLY_OPA_DISP, pauseCtx->questPageVtx,
        ...
        if (pauseCtx->pageIndex != PAUSE_MAP) {
            ...
            gSPMatrix(
                POLY_OPA_DISP++, Matrix_NewMtx(gfxCtx, "../z_kaleido_scope_PAL.c", 1243),
                G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
            );
        ...
        switch (pauseCtx->pageIndex) {
            case PAUSE_ITEM:
                ...
                gSPMatrix(
                    POLY_OPA_DISP++, Matrix_NewMtx(gfxCtx, "../z_kaleido_scope_PAL.c", 1281),
                    G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
                );
            ...
            case PAUSE_MAP:
                ...
                gSPMatrix(
                    POLY_OPA_DISP++, Matrix_NewMtx(gfxCtx, "../z_kaleido_scope_PAL.c", 1303),
                    G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
                );
            ...
            case PAUSE_QUEST:
                ...
                gSPMatrix(
                    POLY_OPA_DISP++, Matrix_NewMtx(gfxCtx, "../z_kaleido_scope_PAL.c", 1343),
                    G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
                );
            ...
            case PAUSE_EQUIP:
                ...
                gSPMatrix(
                    POLY_OPA_DISP++, Matrix_NewMtx(gfxCtx, "../z_kaleido_scope_PAL.c", 1367),
                    G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
                );
    ...
    if ((pauseCtx->state == 7) || ((pauseCtx->state >= 8) && (pauseCtx->state < 0x12))) {
        ...
        gSPMatrix(
            POLY_OPA_DISP++, Matrix_NewMtx(gfxCtx, "../z_kaleido_scope_PAL.c", 1424),
            G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
        );
        ...
        if (((pauseCtx->state == 7) && (pauseCtx->unk_1EC < 4)) || (pauseCtx->state == 0xE)) {
            ...
            gDPSetCombineLERP(
                POLY_OPA_DISP++, 1, 0, PRIMITIVE, 0, TEXEL0, 0, PRIMITIVE, 0, 1, 0, PRIMITIVE, 0, TEXEL0,
                0, PRIMITIVE, 0
            );
            ...
            if ((pauseCtx->state != 0xF) && ((pauseCtx->state == 0x10) || (pauseCtx->state == 0x11))) {
                ...
                gDPSetCombineLERP(
                    POLY_OPA_DISP++, 1, 0, PRIMITIVE, 0, TEXEL0, 0, PRIMITIVE, 0, 1, 0, PRIMITIVE, 0,
...
void KaleidoScope_DrawInfoPanel(PlayState* play) {
    ...
    gSPMatrix(
        POLY_OPA_DISP++, Matrix_NewMtx(play->state.gfxCtx, "../z_kaleido_scope_PAL.c", 1755),
        G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
    );
    ...
    gDPPipeSync(POLY_OPA_DISP++);
    gDPSetCombineLERP(
        POLY_OPA_DISP++, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0, PRIMITIVE,
        ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0
    );
    gDPSetEnvColor(POLY_OPA_DISP++, 20, 30, 40, 0);
    
    if (
        (pauseCtx->state == 6) && (pauseCtx->namedItem != PAUSE_ITEM_NONE) && (pauseCtx->nameDisplayTimer < WREG(89)) &&
        ...
        if (pauseCtx->state == 7) {
            ...
            POLY_OPA_DISP = KaleidoScope_QuadTextureIA8(
                POLY_OPA_DISP, D_8082AD60[gSaveContext.language],
            ...
            if (!pauseCtx->pageIndex) { // pageIndex == PAUSE_ITEM
                ...
                POLY_OPA_DISP = KaleidoScope_QuadTextureIA8(
                    POLY_OPA_DISP, D_8082AD54[gSaveContext.language],
                    D_8082ADD8[gSaveContext.language], 16, 4
                );
            } else if ((pauseCtx->pageIndex == PAUSE_MAP) && sInDungeonScene) {
                
            } else if (
                (pauseCtx->pageIndex == PAUSE_QUEST) && (pauseCtx->cursorSlot[PAUSE_QUEST] >= 6) &&
                ...
                if (pauseCtx->namedItem != PAUSE_ITEM_NONE) {
                    ...
                    POLY_OPA_DISP = KaleidoScope_QuadTextureIA8(
                        POLY_OPA_DISP, D_8082AD6C[gSaveContext.language],
    ...
    if (
        ...
        if (pauseCtx->namedItem != PAUSE_ITEM_NONE) {
            if ((pauseCtx->pageIndex == PAUSE_MAP) && !sInDungeonScene) {
                ...
                DmaMgr_SendRequest1(
                    pauseCtx->nameSegment, gDmaDataTable[16].vromStart + (sp2A * 0x400), 0x400,
                ...
                DmaMgr_SendRequest1(
                    pauseCtx->nameSegment, gDmaDataTable[15].vromStart + (sp2A * 0x400),
...
void KaleidoScope_InitVertices(PlayState* play, GraphicsContext* gfxCtx) {
    ...
    if (
        (pauseCtx->state == 4) || (pauseCtx->state >= 0x12) ||
        ((pauseCtx->state == 7) && ((pauseCtx->unk_1EC == 2) || (pauseCtx->unk_1EC == 5))) ||
        ((pauseCtx->state >= 8) && (pauseCtx->state <= 0xD))
    ) {
        pauseCtx->offsetY = 80; //position of pause menu when folding down
    }
    ...
    for (phi_t2 = 0; phi_t2 < 20; phi_t2++) {
        ...
        pauseCtx->cursorVtx[phi_t2].v.cn[0] = pauseCtx->cursorVtx[phi_t2].v.cn[1] =
            pauseCtx->cursorVtx[phi_t2].v.cn[2] = pauseCtx->cursorVtx[phi_t2].v.cn[3] = 255;//RANK
    }
    ...
    pauseCtx->cursorVtx[1].v.tc[0] = pauseCtx->cursorVtx[2].v.tc[1] = pauseCtx->cursorVtx[3].v.tc[0] =
        ...
        pauseCtx->cursorVtx[15].v.tc[0] = pauseCtx->cursorVtx[15].v.tc[1] = 0x200;//cursor
    
    pauseCtx->cursorVtx[17].v.tc[0] = pauseCtx->cursorVtx[18].v.tc[1] = pauseCtx->cursorVtx[19].v.tc[0] =
        pauseCtx->cursorVtx[19].v.tc[1] = 0x400;//drawing movement of items
...
void KaleidoScope_DrawGameOver(PlayState* play) {
    ...
    gDPSetCombineLERP(
        POLY_OPA_DISP++, TEXEL1, TEXEL0, PRIM_LOD_FRAC, TEXEL0, 0, 0, 0, TEXEL0, PRIMITIVE, ENVIRONMENT,
    ...
    gDPLoadMultiBlock(
        POLY_OPA_DISP++, gGameOverP1Tex, 0, 0, G_IM_FMT_IA, G_IM_SIZ_8b, 64, 32, 0,
        G_TX_WRAP | G_TX_NOMIRROR, G_TX_WRAP | G_TX_NOMIRROR, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
        G_TX_NOLOD
    );
    
    gDPLoadMultiBlock(
        POLY_OPA_DISP++, gGameOverMaskTex, 256, 1, G_IM_FMT_IA, G_IM_SIZ_8b, 64, 32, 0,
    ...
    gSPTextureRectangle(
        POLY_OPA_DISP++, VREG(87) << 2, VREG(88) << 2, (VREG(87) + 64) << 2, (VREG(88) + 32) << 2,
            G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10
    );
    
    gDPLoadMultiBlock(
        POLY_OPA_DISP++, gGameOverP2Tex, 0, 0, G_IM_FMT_IA, G_IM_SIZ_8b, 64, 32, 0,
        G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
        G_TX_NOLOD
    );
    
    gSPTextureRectangle(
        POLY_OPA_DISP++, (VREG(87) + 64) << 2, VREG(88) << 2, (VREG(87) + 128) << 2,
            (VREG(88) + 32) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10
    );
    
    gDPLoadMultiBlock(
        POLY_OPA_DISP++, gGameOverP3Tex, 0, 0, G_IM_FMT_IA, G_IM_SIZ_8b, 64, 32, 0,
        G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
        G_TX_NOLOD
    );
    gSPTextureRectangle(
        POLY_OPA_DISP++, (VREG(87) + 128) << 2, VREG(88) << 2, (VREG(87) + 192) << 2,
            (VREG(88) + 32) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10
    );
    ...
    if (pauseCtx->cursorSpecialPos == 0) {
        ...
        if (pauseCtx->pageIndex == PAUSE_MAP) {
            ...
            if (pauseCtx->cursorSlot[pauseCtx->pageIndex] == 0x18) {
                ...
                (pauseCtx->cursorSlot[pauseCtx->pageIndex] >= 6) &&
...
void KaleidoScope_LoadDungeonMap(PlayState* play) {
    ...
    DmaMgr_SendRequest1(
        interfaceCtx->mapSegment, gDmaDataTable[27].vromStart + (R_MAP_TEX_INDEX * 2040),
        2040, "../z_kaleido_scope_PAL.c", 3467
    );
    
    DmaMgr_SendRequest1(
        interfaceCtx->mapSegment + 0x800,
        gDmaDataTable[27].vromStart + ((R_MAP_TEX_INDEX + 1) * 2040), 2040,
...
void KaleidoScope_Update(PlayState* play) {
    ...
    if (
        (R_PAUSE_MENU_MODE >= 3) && (((pauseCtx->state >= 4) && (pauseCtx->state <= 7)) ||
    ...
    switch (pauseCtx->state) {
        case 3:
            ...
            size0 = gDmaDataTable[7].vromEnd - gDmaDataTable[7].vromStart;
            osSyncPrintf("icon_item size0=%x\n", size0);
            DmaMgr_SendRequest1(
                pauseCtx->iconItemSegment, gDmaDataTable[7].vromStart, size0,
            ...
            size = gDmaDataTable[8].vromEnd - gDmaDataTable[8].vromStart;
            osSyncPrintf("icon_item24 size=%x\n", size);
            DmaMgr_SendRequest1(
                pauseCtx->iconItem24Segment, gDmaDataTable[8].vromStart, size,
            ...
            switch (play->sceneId) {
                ...
                case SCENE_HAKADAN_BS:
                    ...
                    size2 = gDmaDataTable[10].vromEnd - gDmaDataTable[10].vromStart;
                    osSyncPrintf("icon_item_dungeon dungeon-size2=%x\n", size2);
                    DmaMgr_SendRequest1(
                        pauseCtx->iconItemAltSegment, gDmaDataTable[10].vromStart,
                ...
                default:
                    ...
                    size2 = gDmaDataTable[9].vromEnd - gDmaDataTable[9].vromStart;
                    osSyncPrintf("icon_item_field field-size2=%x\n", size2);
                    DmaMgr_SendRequest1(
                        pauseCtx->iconItemAltSegment, gDmaDataTable[9].vromStart,
            ...
            size = gDmaDataTable[12].vromEnd - gDmaDataTable[12].vromStart;
            osSyncPrintf("icon_item_dungeon dungeon-size=%x\n", size);
            DmaMgr_SendRequest1(
                pauseCtx->iconItemLangSegment, gDmaDataTable[12].vromStart, size,
            ...
            if (((void)0, gSaveContext.worldMapArea) < 22) {
                DmaMgr_SendRequest1(
                    pauseCtx->nameSegment + 0x400,
                    gDmaDataTable[16].vromStart +
            ...
            PreRender_SetValuesSave(
                &sPlayerPreRender, PAUSE_EQUIP_PLAYER_WIDTH, PAUSE_EQUIP_PLAYER_HEIGHT,
            ...
            switch (pauseCtx->unk_1E4) {
                ...
                case 5:
                    ...
                    if (CHECK_BTN_ALL(input->press.button, BTN_START)) {
                        ...
                        Audio_PlaySfxGeneral(
                            NA_SE_SY_TRE_BOX_APPEAR, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                            &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb
                        );
                        D_8082B258 = 0;
                        D_8082B25C = 30;
                        ...
                        Audio_PlaySfxGeneral(
                            NA_SE_SY_OCARINA_ERROR, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                            &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb
                        );
                        D_8082B258 = 4;
                        D_8082B25C = 20;
        ...
        case 7:
            switch (pauseCtx->unk_1EC) {
                ...
                case 1:
                    if (CHECK_BTN_ALL(input->press.button, BTN_A)) {
                        if (pauseCtx->promptChoice != 0) {
                            ...
                            Audio_PlaySfxGeneral(
                                NA_SE_SY_PIECE_OF_HEART, &gSfxDefaultPos, 4,
                ...
                case 4:
                    if (
                        CHECK_BTN_ALL(input->press.button, BTN_B) || CHECK_BTN_ALL(input->press.button, BTN_A) ||
        ...
        case 0xA:
            ...
            pauseCtx->iconItemAltSegment = (void*)ALIGN16((u32)pauseCtx->iconItem24Segment + size);
            size2 = gDmaDataTable[11].vromEnd - gDmaDataTable[11].vromStart;
            osSyncPrintf("icon_item_dungeon gameover-size2=%x\n", size2);
            DmaMgr_SendRequest1(
                pauseCtx->iconItemAltSegment, gDmaDataTable[11].vromStart, size2,
        ...
        case 0xE:
            if (CHECK_BTN_ALL(input->press.button, BTN_A)) {
                if (pauseCtx->promptChoice != 0) {
                    ...
                    Audio_PlaySfxGeneral(
                        NA_SE_SY_PIECE_OF_HEART, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                        &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb
                    );
                    pauseCtx->promptChoice = 0;
                    Play_SaveSceneFlags(play);
