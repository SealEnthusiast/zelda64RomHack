...
u32 EffectSsStick_Init(PlayState* play, u32 index, EffectSs* this, void* initParamsx) {
    ...
    StickDrawInfo drawInfo[] = {
        { OBJECT_LINK_BOY, gPlayasAdult_DlBlade3Piece }, // adult, broken sword
        { OBJECT_LINK_CHILD, gPlayasChild_DlDekuStick }, // child, broken stick
    };

    Player* player = GET_PLAYER(play);

    if (player->heldItemActionParam == PLAYER_AP_STICK){
        drawInfo[0].displayList = gPlayasAdult_DlDekuStickBroken;
        drawInfo[1].displayList = gPlayasChild_DlDekuStickBroken;
    }else{
        if (LINK_IS_ADULT){
            drawInfo[0].displayList = gPlayasAdult_DlBlade3Piece;
        }else{
            drawInfo[1].displayList = gPlayasChild_DlBlade3Piece;
        }
    }
